...
class FasterQwen3TTS:
    ...
    def from_pretrained(
        ...
        if backend in ("ggml", "qwentts"):
            # [ComfyUI vendor patch] The optional GGML/qwentts.cpp backend is not
            # vendored. Install the upstream package to use it:
            #   pip install "faster-qwen3-tts[ggml]" --no-deps
            raise RuntimeError(
                "The ggml backend is not included in the bundled copy of "
                "faster-qwen3-tts. Use backend='torch' (the default), or install "
                "the upstream package to get it."
            )

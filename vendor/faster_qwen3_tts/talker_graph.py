    ...
    def _init_cache_layers(self):
        ...
        for layer in self.static_cache.layers:
            if not layer.is_initialized:
                try:
                    layer.lazy_initialization(dummy_k, dummy_k)  # transformers 5.x
                except TypeError:
                    # transformers 4.5x takes only key_states and derives the value
                    # buffer from its shape. Upstream passes the same tensor twice,
                    # so the allocated cache is identical. [ComfyUI vendor patch]
